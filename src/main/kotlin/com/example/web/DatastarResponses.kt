...
sealed class DatastarEvent {
    ...
    data class RemoveFragments(val selector: String) : DatastarEvent() {
        override fun render(): String = "event: datastar-remove-fragments\ndata: selector $selector\n\n"
    }
...
fun DatastarResponse.patchFieldError(
    field: String,
    message: String?,
): DatastarResponse = mergeFragment(renderFieldErrorFragment(field, message))

fun DatastarResponse.patchFormErrors(
    errors: List<FieldError>,
    globalErrors: List<String> = emptyList(),
): DatastarResponse = mergeFragment(renderFormErrorsFragment(errors, globalErrors))

fun DatastarResponse.clearFormErrors(): DatastarResponse =
    apply {
        mergeFragment(renderFormErrorsFragment(emptyList()))
        ConfigFieldIds.forEach { mergeFragment(renderFieldErrorFragment(it, null)) }
    }

fun DatastarResponse.patchSimStatus(
    status: String,
    running: Boolean,
): DatastarResponse = mergeSignals("""{"sim":{"status":"$status","running":$running}}""")
...
fun DatastarResponse.patchLifecycleControls(handle: SimulationHandle?): DatastarResponse =
    apply {
        mergeFragment(renderStatusBadgeFragment(handle))
        mergeFragment(renderLifecycleControlsFragment(handle))
    }

fun DatastarResponse.patchStreamAnchor(handle: SimulationHandle?): DatastarResponse = mergeFragment(renderStreamAnchorFragment(handle))
...
object StreamEventMapper {
    ...
    fun toDatastarEvents(event: SimulationEvent): List<DatastarEvent> =
        when (event) {
            ...
            is SimulationEvent.MetricSample ->
                listOf(
                    DatastarEvent.MergeSignals(buildStatsSignalsJson(event.snapshot)),
                )
            is SimulationEvent.ResponseSample ->
                listOf(
