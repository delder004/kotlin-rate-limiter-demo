...
class WarmupTest {
    @Test
    fun `warmup limiter starts slow and ramps up`() =
        runTest {
    ...
    @Test
    fun `warmup limiter eventually reaches full speed`() =
        runTest {
            val client = createTestClient()
            val limiter =
                SmoothRateLimiter(
    ...
    @Test
    fun `warmup limiter with zero warmup behaves like smooth limiter`() =
        runTest {
            val withWarmup =
                SmoothRateLimiter(
                    permits = 3,
    ...
    @Test
    fun `all requests succeed during warmup period`() =
        runTest {
            val client = createTestClient()
            val limiter =
                SmoothRateLimiter(
