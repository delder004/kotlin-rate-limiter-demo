...
class SimulationRegistryTest {
    ...
    private fun sampleConfig(): SimulationConfig = SimulationConfig(
    ...
    fun `update preserves handle identity and changes config`() {
        val registry = newRegistry()
        val created = registry.create(sampleConfig())
        val newConfig = sampleConfig().copy(
    ...
    @Test
    fun `update from queue to reject mode makes the running engine start denying under overload`() = runTest {
        val alwaysDenied = object : EngineLimiter {
        ...
        val queueConfig = sampleConfig().copy(
            overflowMode = OverflowMode.QUEUE,
    ...
    @Test
    fun `update from low rps to high rps increases producer pressure`() = runTest {
        val granted = object : EngineLimiter {
            override suspend fun acquire() {}
        ...
        val lowConfig = sampleConfig().copy(
