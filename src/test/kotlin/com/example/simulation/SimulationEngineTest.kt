...
class SimulationEngineTest {
    ...
    private fun handle(config: SimulationConfig, logCapacity: Int = 200) =
        SimulationHandle(
            id = "test",
    ...
    @Test
    fun `reject mode increments denials under overload`() = runTest {
        val engine = CoroutineSimulationEngine(
    ...
    @Test
    fun `queue mode accumulates backlog when limiter blocks acquire`() = runTest {
        val engine = CoroutineSimulationEngine(
            scope = backgroundScope,
    ...
    @Test
    fun `queue mode processes requests when limiter grants immediately`() = runTest {
        val engine = CoroutineSimulationEngine(
            scope = backgroundScope,
    ...
    @Test
    fun `latency reflects limiter wait time`() = runTest {
        val engine = CoroutineSimulationEngine(
            scope = backgroundScope,
    ...
    @Test
    fun `stopping engineJob halts further completions`() = runTest {
        val engine = CoroutineSimulationEngine(
            scope = backgroundScope,
            limiterFactory = { AlwaysGrantedLimiter() },
            timeSource = { testScheduler.currentTime },
    ...
    @Test
    fun `bounded log buffer retains capacity and tracks drops`() = runTest {
        val engine = CoroutineSimulationEngine(
            scope = backgroundScope,
            limiterFactory = { AlwaysGrantedLimiter() },
            timeSource = { testScheduler.currentTime },
    ...
    @Test
    fun `droppedIncoming counter is present in snapshot`() = runTest {
        val engine = CoroutineSimulationEngine(
            scope = backgroundScope,
            limiterFactory = { AlwaysGrantedLimiter() },
            timeSource = { testScheduler.currentTime },
