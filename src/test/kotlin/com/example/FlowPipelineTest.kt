...
class FlowPipelineTest {
    @Test
    fun `flow rateLimit extension processes all items`() =
        runTest {
    ...
    @Test
    fun `flow pipeline respects rate limit timing`() =
        runTest {
            val client = createTestClient()
            val limiter = BurstyRateLimiter(permits = 3, per = 1.seconds, timeSource = testScheduler.timeSource)
    ...
    @Test
    fun `flow pipeline can transform and filter with rate limiting`() =
        runTest {
            val client = createTestClient()
